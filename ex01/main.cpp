...
int main()
{
    ...
    ScavTrap named_trap("Bob");
    ScavTrap custom_trap("Eve", 15, 5, 2);
    ScavTrap copy_trap(custom_trap);
    ...
    ScavTrap attacker("Attacker", 10, 2, 5); // HP:10, EP:2, AD:5
    ...
    ScavTrap dead_attacker("DeadAttacker", 0, 10, 5); // HP:0, EP:10, AD:5
    ...
    ScavTrap repairer("Repairer", 10, 2, 0); // HP:10, EP:2
    ...
    ScavTrap dead_repairer("DeadRepairer", 0, 10, 0); // HP:0, EP:10
    ...
    ScavTrap victim("Victim", 10, 10, 0); // HP:10
	...
	// ---5. TEST of guardGate() ---
	std::cout << "--- 5. Testing guardGate() ---" << std::endl;
	ScavTrap default_scav("Default");
	
	default_scav.attack("Enemy");
	default_scav.guardGate();
	std::cout << std::endl;

    std::cout << "--- 6. End of main (Destructors will be called) ---" << std::endl;
    return 0;
}
